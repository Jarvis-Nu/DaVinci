...
import {StdStorage} from "forge-std/src/Components.sol";
...
contract DataDAO {
    ...
    function getSP(uint256 proposalID) internal view returns (address) {
        return proposals[proposalID].storageProvider;
    }

    function isCallerSP(uint256 proposalID) internal view returns (bool) {
        return getSP(proposalID) == msg.sender;
    }

    function isVotingOn(uint256 proposalID) internal view returns (bool) {
        return proposals[proposalID].proposalExpireAt > block.timestamp;
    }

    function createAddCIDProposal(bytes calldata cidraw, uint256 size) public {
